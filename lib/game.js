...
module.exports = class Game {
    ...
    zimo() {
        ...
        if (nukidoraExtracted) {
            // Draw replacement tiles until it is NOT auto-extracted
            let currentCount = newNukidoraCount;
            for (;;) {
                const repl = model.shan.zimo();
                const before = model.shoupai[model.lunban].nukidoraCount;
                model.shoupai[model.lunban].zimo(repl);

                const after = model.shoupai[model.lunban].nukidoraCount;

                if (after > before) {
                    // replacement itself was North; record and loop again
                    model.shan.addNukidora('z4');
                    const addPaipu = { nukidora: { l: model.lunban, p: 'z4', count: after - before } };
                    this.add_paipu(addPaipu);
                    const addMsg = [];
                    for (let l = 0; l < model.fang; l++) addMsg[l] = JSON.parse(JSON.stringify(addPaipu));
                    this.notify_players('nukidora', addMsg);
                    if (this._view) this._view.update(addPaipu);
                    currentCount = after;
                    continue; // draw again
                }

                // Normal tile – send zimo event and finish
                const zimoPaipu = { zimo: { l: model.lunban, p: repl } };
