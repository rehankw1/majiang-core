...
module.exports = class Game {
    ...
    /**
     * 抜きドラ。
     * @param {string} nukidora - 抜く牌（通常は'z4'）。
     */
    nukidora(nukidora = 'z4') {
        const model = this._model;

        // Extract nukidora from player's hand
        const extractedTiles = model.shoupai[model.lunban].extractNukidora();
        ...
        if (extractedTiles.length > 0) {
            ...
            // Create nukidora paipu entry
            const nukidoraPaipu = { 
                nukidora: { 
                    l: model.lunban, 
                    p: nukidora,
                    count: extractedTiles.length
                } 
            };
            ...
            for (let i = 0; i < extractedTiles.length; i++) {
                const replacementTile = model.shan.zimo();
                model.shoupai[model.lunban].zimo(replacementTile);
                
                // Send zimo event for replacement tile
                const zimoPaipu = { zimo: { l: model.lunban, p: replacementTile } };
                this.add_paipu(zimoPaipu);
    ...
    reply_zimo() {
        ...
        else if (reply.nukidora) {
            if (this.allow_nukidora()) {
                this.say('nukidora', model.lunban);
                return this.delay(()=>this.nukidora(reply.nukidora));
            }
        }
        else if (reply.dapai) {
     ...
     */
    allow_pingju() {
        return Game.allow_pingju(this._rule, this._model.shoupai[this._model.lunban], this._diyizimo);
    }

    /**
     * 检查玩家是否可以抜きドラ。
     * @return {boolean} 如果玩家可以抜きドラ则返回true，否则返回false。
     */
    allow_nukidora() {
        const model = this._model;
        return model.shoupai[model.lunban].allow_nukidora();
    }
