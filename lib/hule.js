 ...
 * @param {Array} nukidora - 抜きドラ数组。
 * @return {Array} 和牌后的番种。
 */
function get_post_hupai(shoupai, rongpai, baopai, fubaopai, nukidora) {
    let new_shoupai = shoupai.clone();
    if (rongpai) new_shoupai.zimo(rongpai);
    ...
    // Add nukidora calculation
    let n_nukidora = 0;
    if (nukidora && nukidora.length > 0) {
        n_nukidora = nukidora.length; // Each nukidora tile counts as 1 fan
    }
    if (n_nukidora) post_hupai.push({ name: '抜きドラ', fanshu: n_nukidora });
...
function get_defen(fu, hupai, rongpai, param) {
    ...
    let fang = param.fang || 4; // Default to 4 players if not specified
    ...
    if (hupai[0].fanshu[0] == '*') {
        ...
        if (h) {
            baojia2 = (menfeng + { '+': 1, '=': 2, '-': 3}[h.baojia]) % fang;
    ...
    // Create fenpei array with correct size based on number of players
    let fenpei = new Array(fang).fill(0);
    ...
    if (rongpai || base == 0) {
        ...
        if (fang === 3) {
            ...
            if (menfeng == 0) {
                ...
                // Non-dealer wins: gets zhuangjia from dealer, sanjia from the other player
            ...
            // 4-player tsumo scoring (original logic)
            let zhuangjia = Math.ceil(base * 2 / 100) * 100;
            let sanjia    = Math.ceil(base     / 100) * 100;
            if (menfeng == 0) {
                defen = zhuangjia * 3;
...
function hule(shoupai, rongpai, param) {
    ...
    let post_hupai = get_post_hupai(shoupai, rongpai,
                                    param.baopai, param.fubaopai, param.nukidora);
...
function hule_param(param = {}) {
    let rv = {
        ...
        fang:           param.fang       ?? 4, // Number of players (3 for sanma, 4 for regular)
        hupai: {
            lizhi:      param.lizhi      ?? 0,
        ...
        nukidora:       param.nukidora ? [].concat(param.nukidora) : [],
        jicun: {
