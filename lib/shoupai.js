...
module.exports = class Shoupai {
     ...
     * @param {boolean} addToBingpai 是否将牌添加到bingpai（默认true，但正常游戏流程应该为false）
     * @returns {Shoupai} 更新后的Shoupai对象
     */
    zimo(p, check = true, addToBingpai = true) {
        console.log(`[SHOUPAI-ZIMO] Drawing tile: ${p}, check: ${check}`);
        console.log(`[SHOUPAI-ZIMO] Hand before draw: ${this.toString()}`);
        console.log(`[SHOUPAI-ZIMO] Current zimo: ${this._zimo}`);
        
        if (check && this._zimo) {
            console.log(`[SHOUPAI-ZIMO] ERROR: Already have zimo tile: ${this._zimo}`);
            throw new Error([this, p]);
        }
        
        if (p == '_') {
            console.log(`[SHOUPAI-ZIMO] Drawing blank tile`);
            if (addToBingpai) {
                this._bingpai._++;
            }
            this._zimo = p;
            ...
            if (! Shoupai.valid_pai(p)) {
                console.log(`[SHOUPAI-ZIMO] ERROR: Invalid tile: ${p}`);
                throw new Error(p);
            }
            
            let s = p[0], n = +p[1];
            let bingpai = this._bingpai[s];
            console.log(`[SHOUPAI-ZIMO] Tile suit: ${s}, number: ${n}`);
            console.log(`[SHOUPAI-ZIMO] Current count of ${s}${n}: ${bingpai[n]}`);
            console.log(`[SHOUPAI-ZIMO] Add to bingpai: ${addToBingpai}`);
            
            if (addToBingpai && bingpai[n] == 4) {
                console.log(`[SHOUPAI-ZIMO] ERROR: Already have 4 copies of ${s}${n}`);
                throw new Error([this, p]);
            }
            ...
            if (addToBingpai) {
                ...
                if (n == 0) {
                    if (bingpai[5] == 4) {
                        console.log(`[SHOUPAI-ZIMO] ERROR: Too many 5s with red 5`);
                        throw new Error([this, p]);
                    }
                    bingpai[5]++;
                    console.log(`[SHOUPAI-ZIMO] Red 5 drawn, incremented 5s count`);
                }
                console.log(`[SHOUPAI-ZIMO] Added tile to bingpai`);
            } else {
                console.log(`[SHOUPAI-ZIMO] Tile set as zimo only (not added to bingpai) - for kita replacement`);
            }
            this._zimo = s+n;
        }
        
        console.log(`[SHOUPAI-ZIMO] Hand after draw: ${this.toString()}`);
        console.log(`[SHOUPAI-ZIMO] New zimo tile: ${this._zimo}`);
        return this;
    }
     ...
     */
    dapai(p, check = true) {
        console.log(`[SHOUPAI-DAPAI] Discarding tile: ${p}, check: ${check}`);
        console.log(`[SHOUPAI-DAPAI] Hand before discard: ${this.toString()}`);
        console.log(`[SHOUPAI-DAPAI] Current zimo: ${this._zimo}`);
        
        // Allow discarding even when zimo is null (e.g., after kita extraction)
        // In such cases, we're discarding from bingpai instead of zimo
        if (check && ! this._zimo) {
            console.log(`[SHOUPAI-DAPAI] No zimo tile, discarding from bingpai (e.g., after kita)`);
        }
        
        if (! Shoupai.valid_pai(p)) {
            console.log(`[SHOUPAI-DAPAI] ERROR: Invalid tile: ${p}`);
            throw new Error(p);
        }
        
        let s = p[0], n = +p[1];
        console.log(`[SHOUPAI-DAPAI] Discarding suit: ${s}, number: ${n}`);
        
        this.decrease(s, n);
        console.log(`[SHOUPAI-DAPAI] Decreased tile count for ${s}${n}`);
        
        this._zimo = null;
        console.log(`[SHOUPAI-DAPAI] Cleared zimo tile`);
        
        if (p.slice(-1) == '*') {
            this._lizhi = true;
            console.log(`[SHOUPAI-DAPAI] Riichi declared!`);
        }
        
        console.log(`[SHOUPAI-DAPAI] Hand after discard: ${this.toString()}`);
        return this;
    }
    ...
    /**
     * 抜きドラ（nukidora）- 在三麻中抜出北风牌 (现在只抜出一张)
     * @param {boolean} isStartingHand - 是否为起手kita（不需要清除zimo）
     * @returns {Array<string>} 抜出的牌数组（现在只包含一张牌）
     */
    extractNukidora(isStartingHand = false) {
        console.log(`[EXTRACT-NUKIDORA] Starting nukidora extraction (ONE TILE ONLY), starting hand: ${isStartingHand}`);
        console.log(`[EXTRACT-NUKIDORA] Current hand: ${this.toString()}`);
        console.log(`[EXTRACT-NUKIDORA] Game mode (fang): ${this._fang}`);
        console.log(`[EXTRACT-NUKIDORA] Current zimo tile: ${this._zimo}`);
        
        if (this._fang !== 3) {
            console.log(`[EXTRACT-NUKIDORA] Nukidora not available - not in 3-player mode`);
            return []; // Only in 3-player mode
        }

        const extractedTiles = [];
        const bingpaiZ4Count = this._bingpai.z[4]; // z4 tiles in bingpai
        const zimoIsZ4 = !isStartingHand && this._zimo === 'z4';
        
        console.log(`[EXTRACT-NUKIDORA] North wind tiles (z4) in bingpai: ${bingpaiZ4Count}`);
        console.log(`[EXTRACT-NUKIDORA] Zimo is z4: ${zimoIsZ4}`);
        console.log(`[EXTRACT-NUKIDORA] Current nukidora count: ${this._nukidora.length}`);

        // Check if there are any z4 tiles to extract
        if (bingpaiZ4Count === 0 && !zimoIsZ4) {
            console.log(`[EXTRACT-NUKIDORA] No North wind tiles to extract`);
            return extractedTiles;
        }

        // Extract only ONE z4 tile
        // Priority: extract from zimo first (if it's z4), then from bingpai
        if (zimoIsZ4) {
            // Extract the zimo z4 tile AND also remove the corresponding bingpai copy
            // This fixes the double-counting issue where drawing z4 adds to both bingpai and zimo
            extractedTiles.push('z4');
            this._nukidora.push('z4');
            this._zimo = null; // Clear zimo after extraction
            
            // CRITICAL FIX: Also remove z4 from bingpai to prevent false leftover
            if (this._bingpai.z[4] > 0) {
                this._bingpai.z[4]--; // Remove the double-counted copy from bingpai
                console.log(`[EXTRACT-NUKIDORA] Extracted ONE z4 tile from zimo AND removed double-counted copy from bingpai`);
            } else {
                console.log(`[EXTRACT-NUKIDORA] Extracted ONE z4 tile from zimo (no bingpai copy to remove)`);
            }
        } else if (bingpaiZ4Count > 0) {
            // Extract one z4 from bingpai only
            extractedTiles.push('z4');
            this._nukidora.push('z4');
            this._bingpai.z[4]--; // Remove only ONE z4 tile from bingpai
            console.log(`[EXTRACT-NUKIDORA] Extracted ONE z4 tile from bingpai (${this._bingpai.z[4]} remaining)`);
        }

        console.log(`[EXTRACT-NUKIDORA] Extraction complete - extracted ${extractedTiles.length} tile(s)`);
        console.log(`[EXTRACT-NUKIDORA] New nukidora count: ${this._nukidora.length}`);
        console.log(`[EXTRACT-NUKIDORA] Remaining z4s in bingpai: ${this._bingpai.z[4]}`);
        console.log(`[EXTRACT-NUKIDORA] Hand after extraction: ${this.toString()}`);
     ...
     */
    allow_nukidora() {
        return this._fang === 3 && this.getTotalZ4Count() > 0;
    }

    /**
     * 获取手牌中所有z4牌的总数（包括bingpai和zimo）
     * @returns {number} z4牌的总数
     */
    getTotalZ4Count() {
        let total = this._bingpai.z[4]; // z4 tiles in bingpai
        if (this._zimo === 'z4') {
            total++; // Add zimo z4 if present
        }
        return total;
    }
