...
module.exports = class Shoupai {
     ...
     */
    extractNukidora() {
        // Only used in sanma (3-player) where 北 (z4) is the kita tile
        if (this._fang !== 3) return [];

        // No Norths left → nothing happens
        if (this._bingpai.z[4] === 0) return [];

        // Remove exactly ONE 北 and record it
        this._bingpai.z[4]--;
     ...
     */
    get_nukidora_pai() {
        // If at least one 北 is in hand, present a single option (z4)
        return this.allow_nukidora() ? ['z4'] : [];
    }
